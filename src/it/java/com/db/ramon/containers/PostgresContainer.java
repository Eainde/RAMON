package com.db.ramon.containers;
...
public class PostgresContainer extends GenericContainer<PostgresContainer> {
  ...
  public void start() {
    ...
    System.getProperty(
        "spring.datasource.password", container.getEnvMap().get("POSTGRES_PASSWORD"));
    System.getProperty("spring.datasource.username", container.getEnvMap().get("POSTGRES_USER"));
    LOGGER.info(
        "PostgreSQL started on {} : {}", container.getHost(), container.getMappedPort(MAPPED_PORT));
  }
}
