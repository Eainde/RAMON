...
public class GeneralRestCallSteps {
  ...
  public void givenPath(final String path) {
    ...
    behaviourState.putResult(
        StateConstants.URI_BUILDER, () -> uriComponentsBuilder.replacePath(path));
  }
  ...
  public void sendRequest(final String method) {
    final UriComponentsBuilder uriComponentsBuilder =
        behaviourState.fetchValue(StateConstants.URI_BUILDER, UriComponentsBuilder.class);
    behaviourState.putResult(
        StateConstants.RESPONSE_ENTITY, () -> sendRestCall(method, uriComponentsBuilder));
  }
  ...
  public void responseBodyShouldMatch(final String responseBody) throws IOException {
    ...
    final var expectedMap =
        objectMapper.readTree(
            Resources.toString(
                Resources.getResource(String.format("responseBody/%s.json", responseBody)),
                StandardCharsets.UTF_8));
    assertThat(actualMap, equalTo(expectedMap));
  }

  private ResponseEntity<String> sendRestCall(
      final String method, final UriComponentsBuilder uriComponentsBuilder) {
